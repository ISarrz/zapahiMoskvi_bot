...
async def edit_placemarks_placemark_selector_send_placemarks_menu(update: Update, context: CallbackContext):
    ...
    if sheets and len(sheets) > 1:
        text = (f"📍Мои метки\n"
                f"стр.{context.user_data['placemark_sheet'] + 1}\n\n"
                "Нажмите на кнопку, чтобы увидеть метку целиком, отредактировать или удалить ее.\n\n"
                "Если ваша метка не прошла модерацию и не попала на общую карту, она все равно будет отображаться тут.")
...
async def edit_placemarks_update_tags_menu(update: Update, context: CallbackContext):
    ...
    text = "Выберите теги, нажимая на них. Для возврата к категориям нажмите «Назад»."
